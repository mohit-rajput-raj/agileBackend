...
export const addHistory = async (req, res) => {
  try {
    ...
    const history = await Hist.create({
      ...
      orderId: order.orderId,
      orderStatus: order.orderStatus,
      orderPaid: order.orderPaid || false
    });
    const user = await User.findByIdAndUpdate(
      req.user._id,
      { $push: { "profile.history": history._id } },
...
export const deleteAsk = async (req, res) => {
  ...
  try {
    ...
    console.error("error in deleteAsk", error);
...
export const makeReject = async (req, res) => {
  ...
  try {
    ...
    console.error("Error in makeReject", error);
...
export const AddToDo = async (req, res) => {//////////////////////////
  ...
  try {
    ...
    const history = await Hist.create({
      caption: order.caption,
      description: "order status changed to running",
      orderId: order.orderId,
      orderStatus: "running",
      orderPaid: order.orderPaid || false
    });
...
export const deleteFromCreation = async (req,res)=>{
  try {
    ...
    console.error("error in deleteFromCreation", { error });
    res.status(500).json({ msg: error.message });
  }
}
...
export const deployOrder = async (req, res) => {
  try {
    ...
    const history = await Hist.create({
      caption: order.caption,
      description: `order assigned to user ${person.username}`,
      orderId: order.orderId,
      orderStatus: "waiting",
      orderPaid: order.orderPaid || false
    });
